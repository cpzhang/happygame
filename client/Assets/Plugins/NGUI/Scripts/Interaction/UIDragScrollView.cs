...
public class UIDragScrollView : MonoBehaviour
{
	...
	bool mStarted = false;
    public bool mPageControl = false;
    UIDragPageScrollView uiDragePageScrollView;
	/// <summary>
	...
	void FindScrollView ()
	{
        if (mPageControl)
        {
            uiDragePageScrollView = NGUITools.FindInParents<UIDragPageScrollView>(mTrans);
            // 底板特殊处理
            if (uiDragePageScrollView == null)
            {
                uiDragePageScrollView = mTrans.parent.GetComponentInChildren<UIDragPageScrollView>();
            }
            return;
        }
	...
	void OnPress (bool pressed)
	{
        ...
        if (mPageControl && uiDragePageScrollView != null)
            uiDragePageScrollView.OnItemPressed(pressed);
	}
	...
	{
		if (scrollView && NGUITools.GetActive(this) && !mPageControl)
			scrollView.Drag();
	}
