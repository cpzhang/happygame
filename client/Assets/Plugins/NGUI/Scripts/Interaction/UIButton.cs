...
public class UIButton : UIButtonColor
{
	...
	/// </summary>
    public GameObject click_effect_;
    ...
    protected override void OnPress(bool isPressed)
    {
        ...
        CreateClickEffect();
        if (click_effect_ != null)
        {
            click_effect_.SetActive(isPressed);
        }
    }
    ...
    protected override void OnHover(bool isOver)
    {
        ...
        if (click_effect_ != null)
        {
            click_effect_.SetActive(isOver);
        }
    }
	...
	public override void SetState (State state, bool immediate)
	{
		...
		if (mSprite != null)
		{
			switch (state)
            {
                case State.Normal: SetSprite(mNormalSprite);
                    if (click_effect_ != null)
                    {
                        click_effect_.SetActive(false);
                    };
                    break;
		...
		else if (mSprite2D != null)
        {
            switch (state)
            {
                case State.Normal: SetSprite(mNormalSprite2D);
                    if (click_effect_ != null)
                    {
                        click_effect_.SetActive(false);
                    }; 
    ...
    protected void CreateClickEffect()
    {
        Transform parent_trans = this.transform;
        if (click_effect_ == null)
        {
            GameObject clickeffect_prefab = Resources.Load<GameObject>("ui/CommonUIEffect/ClickEffect");
            if (clickeffect_prefab == null)
            {
                return;
            }
            click_effect_ = NGUITools.AddChild(parent_trans.gameObject, clickeffect_prefab) as GameObject;

			UIWidget parent_trans_sprite = parent_trans.gameObject.GetComponent<UIWidget>();
            if (parent_trans_sprite != null && click_effect_ != null && click_effect_.GetComponent<UIWidget>() != null)
			{
				if (parent_trans_sprite.width <= 150 && parent_trans_sprite.width != parent_trans_sprite.height)
				{
					click_effect_.GetComponent<UIWidget>().width = 260;
					click_effect_.GetComponent<UIWidget>().height = 140;
				}
				if (parent_trans_sprite.width <= 150 && parent_trans_sprite.width == parent_trans_sprite.height)
				{
					click_effect_.GetComponent<UIWidget>().width = 200;
					click_effect_.GetComponent<UIWidget>().height = 200;
				}
				if (parent_trans_sprite.width > 150)
				{
					click_effect_.GetComponent<UIWidget>().width = parent_trans_sprite.width + 200;
					click_effect_.GetComponent<UIWidget>().height = parent_trans_sprite.height + 100;
				}
			}
		}
	}
